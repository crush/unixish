...
impl Screen {
    fn area(&self) -> Rect {
        if self.autohide { self.full } else { self.work }
    }
}
...
fn screens() -> Result<Vec<Screen>> {
    unsafe extern "system" fn call(handle: HMONITOR, _: HDC, _: *mut RECT, data: LPARAM) -> BOOL {
    ...
    let ok = unsafe {
        EnumDisplayMonitors(None, None, Some(call), LPARAM(&mut list as *mut _ as isize))
    };
    if !ok.as_bool() {
